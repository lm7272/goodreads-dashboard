...
def create_composite_image(
    current_book: Path, past_books: list[Path], display_size: tuple[int, int]
) -> Image.Image:
    ...
    current_img = Image.open(current_book).resize((250, 350))
    canvas.paste(current_img, (20, 65))  # Positioned to the left
    ...
    for index, book in enumerate(past_books):
        ...
        x_offset += book_size[0] + 10  # Move right
        if (index + 1) % cols == 0:  # New row
            x_offset = 300
            y_offset += book_size[1] + 10
...
ENVIRONMENT_TO_DISPLAY_FUNCTION: dict[DisplayMode, Callable[[Image.Image, str], None]] = {
